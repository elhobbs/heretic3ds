...
static void InitFonts(void)
{
	...
	FontAEndLump = W_GetNumForName("FONTA_E");
...
void MN_DrTextA(char *text, int x, int y)
{
	...
	while((c = *text++) != 0)
	{
		...
		if(c < 33 || (FontABaseLump + c - 33) >= FontAEndLump)
		{
			x += 5;
		}
		else
		{
			...
			//V_DrawPatch(x, y, p);
			ir_draw_patch_num(x, y, FontABaseLump + c - 33, PU_CACHE);
			x += p->width - 1;
		}
	}
}
...
void MN_Drawer(void)
{
	...
	else
	{
		...
		ir_draw_patch_num(x + SELECTOR_XOFFSET, y, W_GetNumForName(selName), PU_CACHE);
...
static void DrawMainMenu(void)
{
	...
	ir_draw_patch_num(88, 0, W_GetNumForName("M_HTIC"), PU_CACHE);
	ir_draw_patch_num(40, 10, SkullBaseLump + (17 - frame), PU_CACHE);
	ir_draw_patch_num(232, 10, SkullBaseLump + frame, PU_CACHE);
...
static void DrawFileSlots(Menu_t *menu)
{
	...
	for(i = 0; i < 6; i++)
	{
		...
		ir_draw_patch_num(x, y, W_GetNumForName("M_FSLOT"), PU_CACHE);
		if(SlotStatus[i])
		{
...
//              PU_CACHE));
	ir_draw_sreen_num( W_GetNumForName("TITLE") + InfoType, PU_CACHE);
}
...
static void DrawSlider(Menu_t *menu, int item, int width, int slot)
{
	...
	ir_draw_patch_num(x-32, y, W_GetNumForName("M_SLDLT"), PU_CACHE);
	for(x2 = x, count = width; count--; x2 += 8)
	{
		ir_draw_patch_num(x2, y, W_GetNumForName(((count & 1) != 0) ? "M_SLDMD1" : "M_SLDMD2"), PU_CACHE);
		//V_DrawPatch(x2, y, W_CacheLumpName(((count&1)!=0) ? "M_SLDMD1" : "M_SLDMD2", PU_CACHE));
	}
	//V_DrawPatch(x2, y, W_CacheLumpName("M_SLDRT", PU_CACHE));
	ir_draw_patch_num(x2, y, W_GetNumForName("M_SLDRT"), PU_CACHE);

	//V_DrawPatch(x+4+slot*8, y+7, W_CacheLumpName("M_SLDKB", PU_CACHE));
	ir_draw_patch_num(x + 4 + slot * 8, y + 7, W_GetNumForName("M_SLDKB"), PU_CACHE);
}
