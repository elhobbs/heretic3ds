...
void R_InitBuffer (int width, int height)
{
	...
	viewwindowx = (SCREEN_WIDTH-width) >> 1;

	if (width == SCREEN_WIDTH)
		viewwindowy = 0;
	else
		viewwindowy = (SCREEN_HEIGHT-SBARHEIGHT-height) >> 1;
}
...
void R_ExecuteSetViewSize (void)
{
	...
	if (setblocks == 11)
	{
		scaledviewwidth = SCREEN_WIDTH;
		viewheight = SCREEN_HEIGHT;
	}
	else if (setblocks == 10)
	{
		scaledviewwidth = SCREEN_WIDTH;
		viewheight = SCREEN_HEIGHT-SBARHEIGHT+8;
	}
	else
	{
...
//
	...
	pspritescalex = FRACUNIT*viewwidth / SCREENWIDTH;
	pspritescaley = FRACUNIT*viewheight / SCREENHEIGHT;
