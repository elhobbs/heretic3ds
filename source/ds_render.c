...
int viewwidthf = DS_SCREEN_WIDTH << 16;
...
boolean TransformAndClipSegment3(fixed_t vv[2][2], fixed_t ends[2]) {
	float	clip[2][4];
...
void IR_RenderSprites(player_t* player)
{
	...
	while( 1 )
	{
		...
		dsVertex3f(MAP_COORD2(sprite->x + sprite->x1 * yawc), MAP_COORD2(sprite->y + sprite->y1), MAP_COORD2(sprite->z + sprite->x1 * yaws) );

		dsTexCoord2f(sprite->ur, sprite->vt); 
		dsVertex3f(MAP_COORD2(sprite->x + sprite->x2 * yawc), MAP_COORD2(sprite->y + sprite->y1), MAP_COORD2(sprite->z + sprite->x2 * yaws) );

		dsTexCoord2f(sprite->ur, sprite->vb); 
		dsVertex3f(MAP_COORD2(sprite->x + sprite->x2 * yawc), MAP_COORD2(sprite->y + sprite->y2), MAP_COORD2(sprite->z + sprite->x2 * yaws) );

		dsTexCoord2f(sprite->ul, sprite->vb); 
		dsVertex3f(MAP_COORD2(sprite->x + sprite->x1 * yawc), MAP_COORD2(sprite->y + sprite->y2), MAP_COORD2(sprite->z + sprite->x1 * yaws) );
		glEnd();
#endif
		if(sprite->shadow)
		{
...
void IR_RenderPlayerView (player_t* player) {
	...
	glLoadIdentity();
#ifdef ARM9
	gluPerspective(73.74, 256.0 / 192.0, 0.005, 16.0);
#endif
...
#ifdef WIN32
				glTexCoord2f(p[0]/64.0f,p[2] / 64.0f);
				glVertex3f(p[0],p[1],p[2]);

				glTexCoord2f(p1[0] / 64.0f,p1[2] / 64.0f);
				glVertex3f(p1[0],p[1],p1[2]);

				glTexCoord2f(p2[0] / 64.0f,p2[2] / 64.0f);
