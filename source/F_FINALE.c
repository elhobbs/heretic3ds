...
//
	...
	for ( ; count ; count-- )
	{
		...
		if (cx+w->width > SCREENWIDTH)
			break;
		//V_DrawPatch(cx, cy, w);
		ir_draw_patch_num(cx, cy, FontABaseLump + c - 33, PU_CACHE);
		cx += w->width;
	}
	...
	switch(finalestage)
	{
		case 1:
			if(!underwawa)
			{
				...
				//memcpy(screen, W_CacheLumpName("E2END", PU_CACHE),SCREENWIDTH*SCREENHEIGHT);
				ir_draw_sreen_num(W_GetNumForName("E2END"), PU_CACHE);
			}
			paused = false;
			MenuActive = false;
			askforquit = false;
			...
			break;
		case 2:
			//memcpy(screen, W_CacheLumpName("TITLE", PU_CACHE), SCREENWIDTH*SCREENHEIGHT);
			ir_draw_sreen_num(W_GetNumForName("TITLE"), PU_CACHE);
		...
		switch (gameepisode)
		{
			case 1:
				if(shareware)
				{
					...
					ir_draw_sreen_num(W_GetNumForName("ORDER"), PU_CACHE);
				}
				else
				{
					//V_DrawRawScreen(W_CacheLumpName("CREDIT", PU_CACHE));
					ir_draw_sreen_num(W_GetNumForName("CREDIT"), PU_CACHE);
				}
				break;
			case 2:
				F_DrawUnderwater();
