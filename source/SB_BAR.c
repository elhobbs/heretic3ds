...
int PatchLTFACE_num;
int PatchRTFACE_num;
int PatchBARBACK_num;
int PatchCHAIN_num;
int PatchSTATBAR_num;
int PatchLIFEGEM_num;
//patch_t *PatchEMPWEAP;
//patch_t *PatchLIL4BOX;
int PatchLTFCTOP_num;
int PatchRTFCTOP_num;
//patch_t *PatchARMORBOX;
//patch_t *PatchARTIBOX;
int PatchSELECTBOX_num;
//patch_t *PatchKILLSPIC;
//patch_t *PatchMANAPIC;
//patch_t *PatchPOWERICN;
int PatchINVLFGEM1_num;
int PatchINVLFGEM2_num;
int PatchINVRTGEM1_num;
int PatchINVRTGEM2_num;
int PatchINumbers_num[10];
int PatchNEGATIVE_num;
int PatchSmNumbers_num[10];
int PatchBLACKSQ_num;
int PatchINVBAR_num;
int PatchARMCLEAR_num;
int PatchCHAINBACK_num;
...
void SB_Init(void)
{
	...
	PatchLTFACE_num = W_GetNumForName("LTFACE");
	PatchRTFACE_num = W_GetNumForName("RTFACE");
	PatchBARBACK_num = W_GetNumForName("BARBACK");
	PatchINVBAR_num = W_GetNumForName("INVBAR");
	PatchCHAIN_num = W_GetNumForName("CHAIN");

	if(deathmatch)
	{
		PatchSTATBAR_num = W_GetNumForName("STATBAR");
		PatchSTATBAR = W_CacheLumpName("STATBAR", PU_STATIC);
	}
	else
	{
		PatchSTATBAR_num = W_GetNumForName("LIFEBAR");
		PatchSTATBAR = W_CacheLumpName("LIFEBAR", PU_STATIC);
	}
	if(!netgame)
	{ // single player game uses red life gem
		PatchLIFEGEM_num = W_GetNumForName("LIFEGEM2");
	...
	PatchLTFCTOP_num = W_GetNumForName("LTFCTOP");
	PatchRTFCTOP_num = W_GetNumForName("RTFCTOP");
	PatchSELECTBOX_num = W_GetNumForName("SELECTBOX");
	PatchINVLFGEM1_num = W_GetNumForName("INVGEML1");
	PatchINVLFGEM2_num = W_GetNumForName("INVGEML2");
	PatchINVRTGEM1_num = W_GetNumForName("INVGEMR1");
	PatchINVRTGEM2_num = W_GetNumForName("INVGEMR2");
	PatchBLACKSQ_num = W_GetNumForName("BLACKSQ");
	PatchARMCLEAR_num = W_GetNumForName("ARMCLEAR");
	PatchCHAINBACK_num = W_GetNumForName("CHAINBACK");
...
static void DrINumber(signed int val, int x, int y)
{
	...
	if(val < 0)
	{
		if(val < -9)
		{
			...
			ir_draw_patch_num(x + 1, y + 1, W_GetNumForName("LAME"), PU_CACHE);
		}
		else
		{
			val = -val;
			...
			ir_draw_patch_num(x + 18, y, PatchINumbers_num[val], PU_CACHE);
			ir_draw_patch_num(x + 9, y, PatchNEGATIVE_num, PU_CACHE);
		}
		return;
	}
	if(val > 99)
	{
		...
		ir_draw_patch_num(x, y, PatchINumbers_num[val / 100], PU_CACHE);
	}
	val = val%100;
	if(val > 9 || oldval > 99)
	{
		...
		ir_draw_patch_num(x + 9, y, PatchINumbers_num[val / 10], PU_CACHE);
	}
	val = val%10;
...
static void DrBNumber(signed int val, int x, int y)
{
	...
	if(val > 99)
	{
		...
		ir_draw_shadow_patch_num(xpos + 6 - patch->width / 2, y, FontBNumBase + val / 100, PU_CACHE);
	}
	val = val%100;
	xpos += 12;
	if(val > 9 || oldval > 99)
	{
		...
		//V_DrawShadowedPatch(xpos + 6 - patch->width / 2, y, patch);
		ir_draw_shadow_patch_num(xpos + 6 - patch->width / 2, y, FontBNumBase + val / 10, PU_CACHE);
	}
	val = val%10;
	xpos += 12;
	patch = W_CacheLumpNum(FontBNumBase+val, PU_CACHE);
	//V_DrawShadowedPatch(xpos + 6 - patch->width / 2, y, patch);
	ir_draw_shadow_patch_num(xpos + 6 - patch->width / 2, y, FontBNumBase + val, PU_CACHE);
}
...
static void DrSmallNumber(int val, int x, int y)
{
	...
	if(val > 9)
	{
		...
		//V_DrawPatch(x, y, patch);
		ir_draw_patch_num(x, y, PatchSmNumbers_num[val / 10], PU_CACHE);
	}
	val = val%10;
	patch = PatchSmNumbers[val];
	//V_DrawPatch(x + 4, y, patch);
	ir_draw_patch_num(x + 4, y, PatchSmNumbers_num[val], PU_CACHE);
}
...
void SB_Drawer(void)
{
	...
	else
	{
		...
		if(SB_state == -1)
		{
			...
			ir_draw_patch_num(0, 158, PatchBARBACK_num, PU_CACHE);
			if(players[consoleplayer].cheats&CF_GODMODE)
			{
				//V_DrawPatch(16, 167, W_CacheLumpName("GOD1", PU_CACHE));
				//V_DrawPatch(287, 167, W_CacheLumpName("GOD2", PU_CACHE));
				ir_draw_patch_num(16, 167, W_GetNumForName("GOD1"), PU_CACHE);
				ir_draw_patch_num(287, 167, W_GetNumForName("GOD2"), PU_CACHE);
			}
			oldhealth = -1;
		}
		DrawCommonBar();
		if(!inventory)
		{
			if(SB_state != 0)
			{
				...
				ir_draw_patch_num(34, 160, PatchSTATBAR_num, PU_CACHE);
				oldarti = 0;
				oldammo = -1;
				oldarmor = -1;
				oldweapon = -1;
		...
		else
		{
			if(SB_state != 1)
			{
				...
				ir_draw_patch_num(34, 160, PatchINVBAR_num, PU_CACHE);
			}
			DrawInventoryBar();
			SB_state = 1;
		}
			...
			|| !(CPlayer->powers[pw_flight]&16))
		{
			...
			if(CPlayer->mo->flags2&MF2_FLY)
			{
				if(hitCenterFrame && (frame != 15 && frame != 0))
				{
					...
					ir_draw_patch_num(20, 17, spinflylump + 15, PU_CACHE);
				}
				else
				{
					//V_DrawPatch(20, 17, W_CacheLumpNum(spinflylump + frame, PU_CACHE));
					ir_draw_patch_num(20, 17, spinflylump + frame, PU_CACHE);
	...
	if(CPlayer->powers[pw_weaponlevel2] && !CPlayer->chickenTics)
	{
			...
			|| !(CPlayer->powers[pw_weaponlevel2]&16))
		{
			...
			ir_draw_patch_num(300, 17, spinbooklump + frame, PU_CACHE);
			BorderTopRefresh = true;
			UpdateState |= I_MESSAGES;
		}
		else
...
void DrawCommonBar(void)
{
	...
	ir_draw_patch_num(0, 148, PatchLTFCTOP_num, PU_CACHE);
	ir_draw_patch_num(290, 148, PatchRTFCTOP_num, PU_CACHE);
	...
	if(oldhealth != HealthMarker)
	{
		...
		ir_draw_patch_num(0, 190, PatchCHAINBACK_num, PU_CACHE);
		ir_draw_patch_num(2 + (healthPos % 17), chainY, PatchCHAIN_num, PU_CACHE);
		ir_draw_patch_num(17 + healthPos, chainY, PatchLIFEGEM_num, PU_CACHE);
		ir_draw_patch_num(0, 190, PatchLTFACE_num, PU_CACHE);
		ir_draw_patch_num(276, 190, PatchRTFACE_num, PU_CACHE);
		ShadeChain();
...
void DrawMainBar(void)
{
	...
	if(ArtifactFlash)
	{
		...
		ir_draw_patch_num(180, 161, PatchBLACKSQ_num, PU_CACHE);
		ir_draw_patch_num(182, 161, W_GetNumForName("useartia") + ArtifactFlash - 1, PU_CACHE);
		ArtifactFlash--;
		...
		|| oldartiCount != CPlayer->inventory[inv_ptr].count)
	{
		...
		if(CPlayer->readyArtifact > 0)
		{
			...
			ir_draw_patch_num(179, 160, W_GetNumForName(patcharti[CPlayer->readyArtifact]), PU_CACHE);
	...
	if(deathmatch)
	{
		...
		if(temp != oldfrags)
		{
			...
			ir_draw_patch_num(57, 171, PatchARMCLEAR_num, PU_CACHE);
	...
	if(oldkeys != playerkeys)
	{
		if(CPlayer->keys[key_yellow])
		{
			...
			ir_draw_patch_num(153, 164, W_GetNumForName("ykeyicon"), PU_CACHE);
		}
		if(CPlayer->keys[key_green])
		{
			//V_DrawPatch(153, 172, W_CacheLumpName("gkeyicon", PU_CACHE));
			ir_draw_patch_num(153, 172, W_GetNumForName("gkeyicon"), PU_CACHE);
		}
		if(CPlayer->keys[key_blue])
		{
			//V_DrawPatch(153, 180, W_CacheLumpName("bkeyicon", PU_CACHE));
			ir_draw_patch_num(153, 180, W_GetNumForName("bkeyicon"), PU_CACHE);
		}
		oldkeys = playerkeys;
		UpdateState |= I_STATBAR;
	}
	...
	if(oldammo != temp || oldweapon != CPlayer->readyweapon)
	{
		...
		ir_draw_patch_num(108, 161, PatchBLACKSQ_num, PU_CACHE);
		if(temp && CPlayer->readyweapon > 0 && CPlayer->readyweapon < 7)
		{
			DrINumber(temp, 109, 162);
			//V_DrawPatch(111, 172, W_CacheLumpName(ammopic[CPlayer->readyweapon - 1], PU_CACHE));
			ir_draw_patch_num(111, 172, W_GetNumForName(ammopic[CPlayer->readyweapon - 1]), PU_CACHE);
		}
		oldammo = temp;
	...
	if(oldarmor != CPlayer->armorpoints)
	{
		...
		ir_draw_patch_num(224, 171, PatchARMCLEAR_num, PU_CACHE);
...
void DrawInventoryBar(void)
{
	...
	for(i = 0; i < 7; i++)
	{
		...
		{
			//V_DrawPatch(50 + i * 31, 160, W_CacheLumpName(patcharti[CPlayer->inventory[x + i].type], PU_CACHE));
			ir_draw_patch_num(50 + i * 31, 160, W_GetNumForName(patcharti[CPlayer->inventory[x + i].type]), PU_CACHE);
	...
	if(x != 0)
	{
		...
		ir_draw_patch_num(38, 159, !(leveltime & 4) ? PatchINVLFGEM1_num : PatchINVLFGEM2_num, PU_CACHE);
	}
	if(CPlayer->inventorySlotNum-x > 7)
	{
		//V_DrawPatch(269, 159, !(leveltime & 4) ? PatchINVRTGEM1 : PatchINVRTGEM2);
		ir_draw_patch_num(269, 159, !(leveltime & 4) ? PatchINVRTGEM1_num : PatchINVRTGEM2_num, PU_CACHE);
	}
}
...
void DrawFullScreenStuff(void)
{
	...
	if(!inventory)
	{
		if(CPlayer->readyArtifact > 0)
		{
			...
			//V_DrawPatch(286, 170, W_CacheLumpName(patcharti[CPlayer->readyArtifact], PU_CACHE));
			ir_draw_fuzz_patch_num(286, 170, W_GetNumForName("ARTIBOX"), PU_CACHE);
			ir_draw_fuzz_patch_num(286, 170, W_GetNumForName(patcharti[CPlayer->readyArtifact]), PU_CACHE);
	...
	else
	{
		...
		for(i = 0; i < 7; i++)
		{
			...
			ir_draw_fuzz_patch_num(50 + i * 31, 168, W_GetNumForName("ARTIBOX"), PU_CACHE);
			if(CPlayer->inventorySlotNum > x+i
				&& CPlayer->inventory[x+i].type != arti_none)
			{
				//V_DrawPatch(50 + i * 31, 168, W_CacheLumpName(patcharti[CPlayer->inventory[x + i].type], PU_CACHE));
				ir_draw_patch_num(50 + i * 31, 168, W_GetNumForName(patcharti[CPlayer->inventory[x + i].type]), PU_CACHE);
				DrSmallNumber(CPlayer->inventory[x+i].count, 69+i*31, 190);
			}
		}
		//V_DrawPatch(50 + curpos * 31, 191 - 3, PatchSELECTBOX);
		ir_draw_patch_num(50 + curpos * 31, SCREENHEIGHT-3, PatchSELECTBOX_num, PU_CACHE);
		if(x != 0)
		{
			//V_DrawPatch(38, 167, !(leveltime & 4) ? PatchINVLFGEM1 : PatchINVLFGEM2);
			ir_draw_patch_num(38, 167, !(leveltime & 4) ? PatchINVLFGEM1_num : PatchINVLFGEM2_num, PU_CACHE);
		}
		if(CPlayer->inventorySlotNum-x > 7)
		{
			//V_DrawPatch(269, 167, !(leveltime & 4) ? PatchINVRTGEM1 : PatchINVRTGEM2);
			ir_draw_patch_num(269, 167, !(leveltime & 4) ? PatchINVRTGEM1_num : PatchINVRTGEM2_num, PU_CACHE);
		}
	}
}
